...
iframe.src = `http://localhost:5000?${colorSchemeParam}=1&setlang=${navigator.language}&extension=${chrome.runtime.id}`;
...
async function postMessageListner(event) {
  ...
  if (eventName === "Discover.Chat.Interact.Req") {
    ...
    sendEventToIframe("Discover.Chat.Interact.Rep", { status: true, response: data.response });
